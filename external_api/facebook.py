...
class Facebook(object):
    ...
    def posts(self, page_id, limit=100):
        ...
        def get_results():
            while count != len(posts) and len(rpcs) > 0:
                rpc = rpcs.pop()
                rpc.wait()
            return posts, count
        ...
        def create_next_rpc():
            if len(post_ids):
                ...
                if post_id is not None:
                    rpc = self._get_async('{}?fields=id,name,picture,caption,attachments'.format(post_id), handle_result)
                    rpcs.append(rpc)
